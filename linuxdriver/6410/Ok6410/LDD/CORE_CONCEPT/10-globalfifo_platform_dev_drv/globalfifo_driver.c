...
#include <linux/platform_device.h>  /* platform_driver structure */
#include <asm/uaccess.h>            /* copy_from_user/copy_to_user */


#define GLOBALFIFO_SIZE  0x1000  /* global fifo size 4KB */
#define GLOBALFIFO_MAJOR 251     /* The major number */
...
static int __devinit globalfifo_probe(struct platform_device *pdev)
{
    ...
    device_create(globalfifo_cls, NULL, devno, NULL, "globalfifo"); /* /dev/globalfifo */
...
static int __init globalfifo_platform_driver_init(void)
{
    return platform_driver_register(&globalfifo_driver);
}

static void __exit globalfifo_platform_driver_exit(void)
{
    platform_driver_unregister(&globalfifo_driver);
}

module_init(globalfifo_platform_driver_init);
module_exit(globalfifo_platform_driver_exit);

...
static volatile unsigned long *gpldat;
...
static irqreturn_t tiny6410_keys_irq(int irq, void *dev_id)
{
    ...
    if ((irq_dev->irq == IRQ_EINT(19)) || (irq_dev->irq == IRQ_EINT(20)))
        irq_dev->pin_val = *gpldat & (1<<irq_dev->pin);
    else
        irq_dev->pin_val = *gpndat & (1<<irq_dev->pin);
...
{
    gpldat = ioremap(0x7F008818, 4);
    gpndat = ioremap(0x7F008834, 4);
}

static void free_keys_gpio(void)
{
    iounmap(gpldat);
    iounmap(gpndat);
}
...
static int __init tiny6410_input_key_init(void)
{
    ...
    /* Keys GPIO setup(GPNDAT and GPLDAT) */
    setup_keys_gpio();

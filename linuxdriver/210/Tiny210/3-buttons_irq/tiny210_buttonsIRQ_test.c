...
{
    int tiny210_buttonsPoll_fd;
    int tiny210_leds_fd;
    int ubuf[8];

    tiny210_buttonsPoll_fd = open("/dev/tiny210ButtonsIRQ", O_RDWR);
    if (tiny210_buttonsPoll_fd < 0) {
        printf("ERROR: Failed to open \"/dev/tiny210ButtonsIRQ\"!\n");
        return -1;
    }

    tiny210_leds_fd = open("/dev/tiny210Leds", O_RDWR);
    if (tiny210_leds_fd < 0) {
        printf("ERROR: Failed to open \"/dev/tiny210Leds\"!\n");

...
static void dm9k_hash_table(struct net_device *);
...
static const struct net_device_ops dm9k_netdev_ops = {
	...
	.ndo_set_rx_mode	= dm9k_hash_table,
 ...
 */
static void dm9k_hash_table_unlocked(struct net_device *dev)
{
	board_info_t *db = netdev_priv(dev);
...
static void dm9k_hash_table(struct net_device *dev)
{
        board_info_t *db = netdev_priv(dev);
        unsigned long flags;
    
        spin_lock_irqsave(&db->lock, flags);
        dm9k_hash_table_unlocked(dev);

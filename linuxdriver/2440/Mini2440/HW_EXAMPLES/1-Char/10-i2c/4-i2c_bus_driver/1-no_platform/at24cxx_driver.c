...
static struct at24cxx_driver *at24cxx_driverp;
...
static ssize_t at24cxx_read(struct file *filp, char __user *buf, size_t count, 
        loff_t *ppos)
{
    struct at24cxx_driver *drv = filp->private_data;
...
static int at24cxx_driver_rest_setup(struct at24cxx_driver *drv, 
				  ...
				  const struct i2c_device_id *id)
{
    ...
    at24cxx_driverp = kmalloc(sizeof(struct at24cxx_driver), GFP_KERNEL);
    if (!at24cxx_driverp) {
        result = -ENOMEM;
        goto fail_malloc;
    }

    memset(at24cxx_driverp, 0, sizeof(struct at24cxx_driver));

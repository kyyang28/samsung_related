...
{
	if((major = register_chrdev(0, "button_irq_fasync", &button_irq_fops)) < 0) {
		printk(KERN_ERR "unable to register major device number %d\n", major);
		return -EIO;
	}

	button_irq_class = class_create(THIS_MODULE, "button_irq_fasync");
	if(IS_ERR(button_irq_class))
		return PTR_ERR(button_irq_class);

	button_irq_dev = device_create(button_irq_class, NULL, MKDEV(major, 0), NULL, "button_irq_signal"); 
...
{
	unregister_chrdev(major, "button_irq_fasync");

 ...
 *
 * JFFS2 Id: fs-ecos.c,v 1.39 2004/11/11 12:46:17 dwmw2  Exp
 * $Id: fs-ecos.c,v 3.4 2005/01/05 16:19:01 dedekind Exp $
 *
 */
...
static int jffs3_mount(cyg_fstab_entry * fste, cyg_mtab_entry * mte);
static int jffs3_umount(cyg_mtab_entry * mte);
static int jffs3_open(cyg_mtab_entry * mte, cyg_dir dir, const char *name,
		      int mode, cyg_file * fte);
#ifdef CYGOPT_FS_JFFS3_WRITE
static int jffs3_ops_unlink(cyg_mtab_entry * mte, cyg_dir dir,
			    const char *name);
static int jffs3_ops_mkdir(cyg_mtab_entry * mte, cyg_dir dir, const char *name);
static int jffs3_ops_rmdir(cyg_mtab_entry * mte, cyg_dir dir, const char *name);
static int jffs3_ops_rename(cyg_mtab_entry * mte, cyg_dir dir1,
			    const char *name1, cyg_dir dir2, const char *name2);
static int jffs3_ops_link(cyg_mtab_entry * mte, cyg_dir dir1, const char *name1,
			  cyg_dir dir2, const char *name2, int type);
#endif
static int jffs3_opendir(cyg_mtab_entry * mte, cyg_dir dir, const char *name,
			 cyg_file * fte);
static int jffs3_chdir(cyg_mtab_entry * mte, cyg_dir dir, const char *name,
		       cyg_dir * dir_out);
static int jffs3_stat(cyg_mtab_entry * mte, cyg_dir dir, const char *name,
		      struct stat *buf);
static int jffs3_getinfo(cyg_mtab_entry * mte, cyg_dir dir, const char *name,
			 int key, void *buf, int len);
static int jffs3_setinfo(cyg_mtab_entry * mte, cyg_dir dir, const char *name,
			 int key, void *buf, int len);

// File operations
static int jffs3_fo_read(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio);
#ifdef CYGOPT_FS_JFFS3_WRITE
static int jffs3_fo_write(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio);
#endif
static int jffs3_fo_lseek(struct CYG_FILE_TAG *fp, off_t * pos, int whence);
static int jffs3_fo_ioctl(struct CYG_FILE_TAG *fp, CYG_ADDRWORD com,
			  CYG_ADDRWORD data);
static int jffs3_fo_fsync(struct CYG_FILE_TAG *fp, int mode);
static int jffs3_fo_close(struct CYG_FILE_TAG *fp);
static int jffs3_fo_fstat(struct CYG_FILE_TAG *fp, struct stat *buf);
static int jffs3_fo_getinfo(struct CYG_FILE_TAG *fp, int key, void *buf,
			    int len);
static int jffs3_fo_setinfo(struct CYG_FILE_TAG *fp, int key, void *buf,
			    int len);

// Directory operations
static int jffs3_fo_dirread(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio);
static int jffs3_fo_dirlseek(struct CYG_FILE_TAG *fp, off_t * pos, int whence);


static int jffs3_read_inode (struct _inode *inode);
static void jffs3_clear_inode (struct _inode *inode);
...
#ifdef CYGOPT_FS_JFFS3_WRITE
FSTAB_ENTRY(jffs3_fste, "jffs3", 0,
	    ...
	    jffs3_ops_link,
	    jffs3_opendir,
	    jffs3_chdir, jffs3_stat, jffs3_getinfo, jffs3_setinfo);
#else
FSTAB_ENTRY(jffs3_fste, "jffs3", 0,
	    CYG_SYNCMODE_FILE_FILESYSTEM | CYG_SYNCMODE_IO_FILE,
	    jffs3_mount,
...
static cyg_fileops jffs3_fileops = {
	jffs3_fo_read,
#ifdef CYGOPT_FS_JFFS3_WRITE
	jffs3_fo_write,
#else
...
static cyg_fileops jffs3_dirops = {
	jffs3_fo_dirread,
...
static unsigned char n_fs_mounted = 0;  // a counter to track the number of jffs3 instances mounted

//==========================================================================
// Directory operations

struct jffs3_dirsearch {
...
typedef struct jffs3_dirsearch jffs3_dirsearch;
...
static void init_dirsearch(jffs3_dirsearch * ds,
...
static int find_entry(jffs3_dirsearch * ds)
{
	...
	d = jffs3_lookup(dir, name, namelen);
...
static int jffs3_find(jffs3_dirsearch * d)
{
	int err;

	D2(printf("jffs3_find for path =%s\n", d->path));
...
static int jffs3_pathconf(struct _inode *node, struct cyg_pathconf_info *info)
{
	int err = ENOERR;
	D2(printf("jffs3_pathconf\n"));
...
// filesystem.
static int jffs3_read_super(struct super_block *sb)
{
	...
	D1(printk(KERN_DEBUG "jffs3: read_super\n"));
	...
	if (err != ENOERR) {
		D1(printf
		   ("jffs3: cyg_io_get_config failed to get dev size: %d\n",
		    err));
		return err;
	}
	len = sizeof (bs);
	...
	if (err != ENOERR) {
		D1(printf
		   ("jffs3: cyg_io_get_config failed to get block size: %d\n",
		    err));
		return err;
	}
	...
	c->cleanmarker_size = sizeof(struct jffs3_unknown_node);

	err = jffs3_do_mount_fs(c);
	if (err)
		return -err;

	D1(printk(KERN_DEBUG "jffs3_read_super(): Getting root inode\n"));
	sb->s_root = jffs3_iget(sb, 1);
	if (IS_ERR(sb->s_root)) {
...
static int jffs3_mount(cyg_fstab_entry * fste, cyg_mtab_entry * mte)
{
	...
	struct super_block *jffs3_sb = NULL;
	struct jffs3_sb_info *c;
	cyg_mtab_entry *m;
	cyg_io_handle_t t;
	Cyg_ErrNo err;

	D2(printf("jffs3_mount\n"));
	...
	for (m = &cyg_mtab[0]; m != &cyg_mtab_end; m++) {
		...
		if (m->valid && strcmp(m->fsname, "jffs3") == 0 &&
		    strcmp(m->devname, mte->devname) == 0) {
			jffs3_sb = (struct super_block *) m->data;
		}
	}

	if (jffs3_sb == NULL) {
		jffs3_sb = malloc(sizeof (struct super_block));

		if (jffs3_sb == NULL)
			return ENOMEM;

		c = JFFS3_SB_INFO(jffs3_sb);
		memset(jffs3_sb, 0, sizeof (struct super_block));
		jffs3_sb->s_dev = t;

		c->inocache_list = malloc(sizeof(struct jffs3_inode_cache *) * INOCACHE_HASHSIZE);
		if (!c->inocache_list) {
			free(jffs3_sb);
			return ENOMEM;
		}
		memset(c->inocache_list, 0, sizeof(struct jffs3_inode_cache *) * INOCACHE_HASHSIZE);
                if (n_fs_mounted++ == 0) {
                        jffs3_create_slab_caches(); // No error check, cannot fail
			jffs3_compressors_init(); 
		}

		err = jffs3_read_super(jffs3_sb);
		...
		jffs3_sb->s_root->i_parent = jffs3_sb->s_root;	// points to itself, no dotdot paths above mountpoint
		jffs3_sb->s_root->i_cache_prev = NULL;	// root inode, so always null
		jffs3_sb->s_root->i_cache_next = NULL;
		jffs3_sb->s_root->i_count = 1;	// Ensures the root inode is always in ram until umount

		D2(printf("jffs3_mount erasing pending blocks\n"));
#ifdef CYGOPT_FS_JFFS3_WRITE
	...
	mte->data = (CYG_ADDRWORD) jffs3_sb;

	jffs3_sb->s_mount_count++;
	mte->root = (cyg_dir) jffs3_sb->s_root;
	D2(printf("jffs3_mounted superblock at %x\n", mte->root));
...
static int jffs3_umount(cyg_mtab_entry * mte)
{
	...
	struct super_block *jffs3_sb = root->i_sb;
	struct jffs3_sb_info *c = JFFS3_SB_INFO(jffs3_sb);
        struct jffs3_full_dirent *fd, *next;

	D2(printf("jffs3_umount\n"));

	// Only really umount if this is the only mount
	if (jffs3_sb->s_mount_count == 1) {
...
#endif
		jffs3_iput(root);	// Time to free the root inode

		// free directory entries
		for (fd = root->jffs3_i.dents; fd; fd = next) {
		  next=fd->next;
		...
		// That's all folks.
		D2(printf("jffs3_umount No current mounts\n"));
	} else {
		jffs3_sb->s_mount_count--;
        }
        if (--n_fs_mounted == 0) {
		      ...
		      int mode, cyg_file * file)
{
	...
	if (err == ENOENT) {
		...
		if (ds.last && (mode & O_CREAT)) {
			...
			err = jffs3_create(ds.dir, ds.name, S_IRUGO|S_IXUGO|S_IWUSR|S_IFREG, &node);
...
#ifdef CYGOPT_FS_JFFS3_WRITE
	if (mode & O_TRUNC) {
		struct jffs3_inode_info *f = JFFS3_INODE_INFO(node);
		struct jffs3_sb_info *c = JFFS3_SB_INFO(node->i_sb);
		// If the O_TRUNC bit is set we must clean out the file data.

		node->i_size = 0;
		jffs3_truncate_fraglist(c, &f->fragtree, 0);
		// Update file times
...
static int jffs3_ops_unlink(cyg_mtab_entry * mte, cyg_dir dir, const char *name)
{
	jffs3_dirsearch ds;
	int err;

	D2(printf("jffs3_ops_unlink\n"));
	...
	err = jffs3_unlink(ds.dir, ds.node, ds.name);
	jffs3_iput(ds.dir);
	jffs3_iput(ds.node);
...
static int jffs3_ops_mkdir(cyg_mtab_entry * mte, cyg_dir dir, const char *name)
{
	...
	if (err == ENOENT) {
		if (ds.last) {
			...
			err = -jffs3_mkdir(ds.dir, ds.name, S_IRUGO|S_IXUGO|S_IWUSR);
		}
...
static int jffs3_ops_rmdir(cyg_mtab_entry * mte, cyg_dir dir, const char *name)
{
	...
	err = jffs3_rmdir(ds.dir, ds.node, ds.name);
...
static int jffs3_ops_rename(cyg_mtab_entry * mte, cyg_dir dir1,
			    const char *name1, cyg_dir dir2, const char *name2)
{
	jffs3_dirsearch ds1, ds2;
	int err;

	D2(printf("jffs3_ops_rename\n"));
	...
	if (ds2.node) {
		...
		err = -jffs3_unlink(ds2.dir, ds2.node, ds2.name);
	...
	err = -jffs3_rename(ds1.dir, ds1.node, ds1.name, ds2.dir, ds2.name);
			  ...
			  cyg_dir dir2, const char *name2, int type)
{
	...
	err = jffs3_link(ds1.node, ds2.dir, ds2.name);
			 ...
			 int key, void *buf, int len)
{
	...
	switch (key) {
	case FS_INFO_CONF:
		err = jffs3_pathconf(ds.node, (struct cyg_pathconf_info *) buf);
		break;
...
static int jffs3_fo_read(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
	...
	for (i = 0; i < uio->uio_iovcnt && pos < inode->i_size; i++) {
		...
		D2(printf("jffs3_fo_read inode size %d\n", inode->i_size));

		ret =
		    jffs3_read_inode_range(c, f,
					   (unsigned char *) iov->iov_base, pos,
					   len);
		if (ret) {
			D1(printf
			   ("jffs3_fo_read(): read_inode_range failed %d\n",
			    ret));
			uio->uio_resid = resid;
			up(&f->sem);
			return -ret;
...
// jffs3_fo_write()
// Write data to file.
static int jffs3_extend_file (struct _inode *inode, struct jffs3_raw_inode *ri,
...
static int jffs3_fo_write(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
	...
	if (pos > inode->i_size) {
		int err;
		ri.version = cpu_to_je32(++f->highest_version);
		err = jffs3_extend_file(inode, &ri, pos);
		if (err)
			return -err;
	}
	...
	for (i = 0; i < uio->uio_iovcnt; i++) {
		...
		D2(printf("jffs3_fo_write page_start_pos %d\n", pos));
		D2(printf("jffs3_fo_write transfer size %d\n", l));

		err = jffs3_write_inode_range(c, f, &ri, buf,
					      pos, len, &writtenlen);
		if (err)
			return -err;
...
static int jffs3_fo_lseek(struct CYG_FILE_TAG *fp, off_t * apos, int whence)
{
	struct _inode *node = (struct _inode *) fp->f_data;
	off_t pos = *apos;

	D2(printf("jffs3_fo_lseek\n"));
			    ...
			    int len)
{
	...
	D2(printf("jffs3_fo_getinfo\n"));

	switch (key) {
	case FS_INFO_CONF:
		err = jffs3_pathconf(node, (struct cyg_pathconf_info *) buf);
		break;
...
static int jffs3_fo_dirread(struct CYG_FILE_TAG *fp, struct CYG_UIO_TAG *uio)
{
	...
	D1(printk
	   (KERN_DEBUG "jffs3_readdir() for dir_i #%lu\n", d_inode->i_ino));
				   ...
				   unsigned long *priv)
{
	...
	ret = jffs3_read_inode_range(c, f, gc_buffer, 
...
static struct _inode *new_inode(struct super_block *sb)
{
	...
	// Only called in write.c jffs3_new_inode
	...
	inode->i_ino = 1;
	inode->i_count = 1;
	inode->i_nlink = 1;	// Let JFFS3 manage the link count
	inode->i_size = 0;
...
{
	// Called in super.c jffs3_read_super, dir.c jffs3_lookup,
	// and gc.c jffs3_garbage_collect_pass
	...
	err = jffs3_read_inode(inode);
	if (err) {
		printf("jffs3_read_inode() failed\n");
		jffs3_iput(inode);
		inode = NULL;
		return ERR_PTR(err);
	}
...
{
	// Called in jffs3_find 
	// (and jffs3_open and jffs3_ops_mkdir?)
	// super.c jffs3_read_super,
	// and gc.c jffs3_garbage_collect_pass
 recurse:
	if (!i) {
		printf("jffs3_iput() called with NULL inode\n");
		// and let it fault... 
	}
...
/* jffs3_new_inode: allocate a new inode and inocache, add it to the hash,
...
struct _inode *jffs3_new_inode (struct _inode *dir_i, int mode, struct jffs3_raw_inode *ri)
{
	...
	ret = jffs3_do_new_inode (c, f, mode, ri);
	if (ret) {
		jffs3_iput(inode);
		return ERR_PTR(ret);
	}
						     ...
						     int inum, int nlink)
{
	...
	if (!nlink) {
		/* The inode has zero nlink but its nodes weren't yet marked
		   ...
		   the final (close() and) jffs3_iput() to happen.

		   There's a possibility that the final jffs3_iput() could have 
		   happened while we were contemplating. In order to ensure
		   that we don't cause a new read_inode() (which would fail)
		   for the inode in question, we use ilookup() in this case
		   instead of jffs3_iget().
		...
		*/
		...
		/* Inode has links to it still; they're not going away because
		   jffs3_do_unlink() would need the alloc_sem and we have it.
		   Just jffs3_iget() it, and if read_inode() is necessary that's OK.
		*/
		inode = jffs3_iget(OFNI_BS_2SFFJ(c), inum);
		if (IS_ERR(inode))
			return (void *)inode;
	}
...
uint32_t jffs3_from_os_mode(uint32_t osmode)
{
	...
	printf("os_to_jffs3_mode() cannot convert 0x%x\n", osmode);
	BUG();
	return 0;
}
...
uint32_t jffs3_to_os_mode (uint32_t jmode)
{
	...
	printf("jffs3_to_os_mode() cannot convert 0x%x\n", osmode);
	BUG();
	return 0;
}

 ...
 *
 * JFFS2 Id: flashio.c,v 1.1 2003/11/26 14:09:29 dwmw2  Exp
 * $Id: flashio.c,v 3.2 2005/01/05 16:19:01 dedekind Exp $
 *
 */
...
cyg_bool jffs3_flash_read(struct jffs3_sb_info * c,
			  ...
			  size_t * return_size, char *write_buffer)
{
	...
	//D2(printf("read address = %x\n", CYGNUM_FS_JFFS3_BASE_ADDRESS + read_buffer_offset));
...
cyg_bool jffs3_flash_write(struct jffs3_sb_info * c,
			   ...
			   size_t * return_size, char *read_buffer)
{
	...
	//    D2(printf("write address = %x\n", CYGNUM_FS_JFFS3_BASE_ADDRESS + write_buffer_offset));
	//    D2(printf("read address = %x\n", read_buffer));
	//    D2(printf("size = %x\n", size));
...
int
jffs3_flash_direct_writev(struct jffs3_sb_info *c, const struct iovec *vecs,
	...
	for (i = 0; i < count; i++) {
		...
		if (((vecs[i].iov_len & (sizeof (int) - 1))) ||
			...
			if ((i + 1) < count || vecs[i].iov_len > 256) {
				...
				ret =
				    jffs3_flash_write(c, to, sizetomalloc,
						      &thislen, cbuf);
				...
				ret =
				    jffs3_flash_write(c, to, lentowrite,
						      &thislen, (char *) &buf);
			...
			ret =
			    jffs3_flash_write(c, to, vecs[i].iov_len, &thislen,
					      vecs[i].iov_base);
		totlen += thislen;
...
cyg_bool jffs3_flash_erase(struct jffs3_sb_info * c,
			   struct jffs3_eraseblock * jeb)
{
	...
	//        D2(printf("erase address = %x\n", CYGNUM_FS_JFFS3_BASE_ADDRESS + jeb->offset));
	//        D2(printf("size = %x\n", c->sector_size));

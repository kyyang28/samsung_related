 ...
 * jffs3 directory.
 *
 * JFFS2 Id: compr_lzo.c,v 1.3 2004/06/23 16:34:39 havasi  Exp
 * $Id: compr_lzo.c,v 3.3 2005/01/05 16:18:59 dedekind Exp $
 *
 */

/*
   LZO1X-1 (and -999) compression module for jffs3
...
/*#define BLOCKSIZE		JFFS3_PAGE_SIZE
#define OUTBLOCKSIZE	(BLOCKSIZE + BLOCKSIZE / 64 + 16 + 3)*/

int jffs3_lzo_compress (unsigned char *input,
			unsigned char *output, uint32_t *sourcelen,
			uint32_t *dstlen, void *model);

int jffs3_lzo_decompress (unsigned char *input,
			  unsigned char *output, uint32_t sourcelen,
			  uint32_t dstlen, void *model);

static struct jffs3_compressor jffs3_lzo_comp = {
	.priority = JFFS3_LZO_PRIORITY,
	.name = "lzo",
	.compr = JFFS3_COMPR_LZO,
	.compress = &jffs3_lzo_compress,
	.decompress = &jffs3_lzo_decompress,
#ifdef JFFS3_LZO_DISABLED
	.disabled = 1,
#else
	.disabled = 0,
#endif
...
int jffs3_lzo_init (void)
{
        ...
        if (!wrkmem) return -1;
        jffs3_register_compressor(&jffs3_lzo_comp);
        return 0;
}

void jffs3_lzo_exit (void)
{
	jffs3_unregister_compressor (&jffs3_lzo_comp);

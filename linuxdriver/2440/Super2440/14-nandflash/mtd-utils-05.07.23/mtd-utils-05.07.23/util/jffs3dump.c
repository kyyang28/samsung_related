 ...
 *        summary support by University of Szeged, Hungary
 *
 * $Id: jffs3dump.c,v 1.1 2005/01/21 11:52:23 havasi Exp $
 *
 ...
 *
 * Overview:
 *   This utility dumps the contents of a binary JFFS3 image 
 *
 *
 * Bug/ToDo:
 */
...
{
	printf("Usage: jffs3dump [OPTION] INPUTFILE\n"
	       "Dumps the contents of a binary JFFS3 image.\n"
	       "\n"
...
void do_dumpcontent (void) 
{
	...
	while ( p < (data + imglen)) {
		...
		case JFFS3_NODETYPE_SUMMARY:{
			...
			crc = crc32 (0, node, sizeof (struct jffs3_summary_node) - 8);
			...
			crc = crc32(0, p + sizeof (struct jffs3_summary_node),  je32_to_cpu (node->s.totlen) - sizeof(struct jffs3_summary_node));
			...
			if(verbose){
				...
				sp = (struct jffs3_sum_unknown_flash *) (p + sizeof (struct jffs3_summary_node));
				...
				sm = (struct jffs3_sum_marker *) ((char *)p + je32_to_cpu(node->s.totlen) - sizeof(struct jffs3_sum_marker));
...
{
	char			*p = data;
	union jffs3_node_union 	*node, newnode;
	int			fd, len;
	jint32_t		mode;
	uint32_t		crc;
	...
	while ( p < (data + imglen)) {
		...
		case JFFS3_NODETYPE_INODE:
			...
			newnode.i.hdr_crc = cpu_to_e32 (crc32 (0, &newnode, sizeof (struct jffs3_unknown_node) - 4));
			...
			if (recalccrc) {
				...
				newnode.i.data_crc = cpu_to_e32 ( crc32(0, p + sizeof (struct jffs3_raw_inode), len));
			} else
				newnode.i.data_crc = cnv_e32 (node->i.data_crc);
			
			newnode.i.node_crc = cpu_to_e32 (crc32 (0, &newnode, sizeof (struct jffs3_raw_inode) - 8));
			
			write (fd, &newnode, sizeof (struct jffs3_raw_inode));
			write (fd, p + sizeof (struct jffs3_raw_inode), PAD (je32_to_cpu (node->i.totlen) -  sizeof (struct jffs3_raw_inode)));
		...
		case JFFS3_NODETYPE_DIRENT:
			...
			newnode.d.hdr_crc = cpu_to_e32 (crc32 (0, &newnode, sizeof (struct jffs3_unknown_node) - 4));
			...
			newnode.d.node_crc = cpu_to_e32 (crc32 (0, &newnode, sizeof (struct jffs3_raw_dirent) - 8));
			if (recalccrc)
				newnode.d.name_crc = cpu_to_e32 ( crc32(0, p + sizeof (struct jffs3_raw_dirent), node->d.nsize));
			else
				newnode.d.name_crc = cnv_e32 (node->d.name_crc);

			write (fd, &newnode, sizeof (struct jffs3_raw_dirent));
			write (fd, p + sizeof (struct jffs3_raw_dirent), PAD (je32_to_cpu (node->d.totlen) -  sizeof (struct jffs3_raw_dirent)));
			p += PAD(je32_to_cpu (node->d.totlen));						
			break;
		...
		case JFFS3_NODETYPE_PADDING:
			...
			newnode.u.hdr_crc = cpu_to_e32 (crc32 (0, &newnode, sizeof (struct jffs3_unknown_node) - 4));
			
			write (fd, &newnode, sizeof (struct jffs3_unknown_node));
			len = PAD(je32_to_cpu (node->u.totlen) - sizeof (struct jffs3_unknown_node));
			if (len > 0) 
				write (fd, p + sizeof (struct jffs3_unknown_node), len);
				
			p += PAD(je32_to_cpu (node->u.totlen));						
			break;

		case JFFS3_NODETYPE_SUMMARY : {
			struct jffs3_sum_marker *sm_ptr;
			int i,sum_len;
			int counter = 0;
			...
			newnode.s.hdr_crc = cpu_to_e32 (crc32 (0, &newnode, sizeof (struct jffs3_unknown_node) - 4));
			newnode.s.sum_num = cnv_e16 (node->s.sum_num);
			newnode.s.cln_mkr = cnv_e32 (node->s.cln_mkr);
			newnode.s.padded = cnv_e32 (node->s.padded);
			
			newnode.s.node_crc = cpu_to_e32 (crc32 (0, &newnode, sizeof (struct jffs3_summary_node) - 8));
			
			// summary header
			p += sizeof (struct jffs3_summary_node);
			
			// summary data
			sum_len = je32_to_cpu (node->s.totlen) - sizeof (struct jffs3_summary_node) - sizeof (struct jffs3_sum_marker);
			
			for (i=0; i<je16_to_cpu (node->s.sum_num); i++) {
				union jffs3_sum_flash *fl_ptr;
				
				fl_ptr = (union jffs3_sum_flash *) p;
				...
				switch (je16_to_cpu (fl_ptr->u.nodetype)) {
					case JFFS3_NODETYPE_INODE : {
						...
						p += sizeof (struct jffs3_sum_inode_flash);
							
						counter += sizeof (struct jffs3_sum_inode_flash);
						break;
					}
					case JFFS3_NODETYPE_DIRENT : {
						...
						p += sizeof (struct jffs3_sum_dirent_flash) + fl_ptr->d.nsize;
						counter += sizeof (struct jffs3_sum_dirent_flash) + fl_ptr->d.nsize;
			...
			// summary marker
			sm_ptr = (struct jffs3_sum_marker *) p;
			sm_ptr->offset = cnv_e32 (sm_ptr->offset);
			sm_ptr->erase_size = cnv_e32 (sm_ptr->erase_size);
			sm_ptr->magic = cnv_e32 (sm_ptr->magic);
			p += sizeof (struct jffs3_sum_marker);
			
			// generate new crc on sum data
			newnode.s.sum_crc = cpu_to_e32 ( crc32(0, ((char *) node) + sizeof (struct jffs3_summary_node), 
				je32_to_cpu (node->s.totlen) - sizeof (struct jffs3_summary_node)));
			
			// write out new node header
			write(fd, &newnode, sizeof (struct jffs3_summary_node));
			// write out new summary data
			write(fd, &node->s.sum, sum_len + sizeof (struct jffs3_sum_marker));

...
struct tag_core {
	unsigned long flags;		/* bit 0 = read-only */
...
struct tag_ramdisk {
	unsigned long flags;	/* bit 0 = load, bit 1 = prompt */
	unsigned long size;	/* decompressed ramdisk size in _kilo_ bytes */
	unsigned long start;	/* starting block of floppy-based RAM disk image */
};
...
struct tag_initrd {
	unsigned long start;	/* physical start address */
	unsigned long size;	/* size of compressed ramdisk image in bytes */
};

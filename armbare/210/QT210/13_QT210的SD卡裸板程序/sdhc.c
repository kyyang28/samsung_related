...
U8 SDHC_Init(void)
{
	...
	SDHC_ResetController(sCh); //复位一下SD卡控制器
	...
	// 发CMD7 ---- card Selection
	if ( !SDHC_IssueCommand( sCh, 7, (U32)(sCh->m_uRca<<16), SDHC_CMD_AC_TYPE, SDHC_RES_R1B_TYPE ) )
		return FALSE;
	
	 if ( sCh->m_eCardType == SDHC_SD_CARD ) { //在卡的角度设置卡的速度模式
		SDHC_GetSdScr(sCh);
		if (sCh->m_ucSpecVer==1||sCh->m_ucSpecVer==2) { // 因为sCh->m_ucSpecVer==2，所以执行该分支
	...
	// host controller speed setting.--- 根据卡的速度模式，设置主机的时钟和速度模式等
	speed = ( uOperFreq > SDHC_MMC_HIGH_SPEED_CLOCK) ? (SDHC_HIGH_SPEED) : (SDHC_NORMAL_SPEED);
	SDHC_SetHostCtrlSpeedMode( speed, sCh ); //设置成正常模式

	SDHC_SetSdClockOnOff(0, sCh); // If the sd clock is to be changed, you need to stop sd-clock.
	SDHC_SetSdClock(sCh, speed); // 修改时钟,将SDCLK改为23.825MHz

	// After a card was selected, then the bus width can be changed.
	if (!SDHC_SetDataTransferWidth( sCh)) //设置成4位的位宽
		return FALSE;

	if (!SDHC_WaitForCard2TransferState(sCh)) //等待卡进去发送状态
		return FALSE;

	// CMD16 can not be set while card is in programming state.
	if (!SDHC_IssueCommand(sCh, 16, 512, SDHC_CMD_AC_TYPE, SDHC_RES_R1_TYPE ) ) // 还原卡的块大小是512字节
		return FALSE;

	SDOutp32( sCh->m_uBaseAddr+SDHC_CONTROL2, SDInp32(sCh->m_uBaseAddr+SDHC_CONTROL2)|(1<<8)|(2<<9)|(1<<28)); //使能时钟，调整滤波，取卡条件
	return TRUE;
}
...
U8 SDHC_SetSDOCR(SDHC* sCh)
{
	...
	// 发CMD0 ---- Place all cards in the idle state.
	if (!SDHC_IssueCommand( sCh, 0, 0, SDHC_CMD_BC_TYPE, SDHC_RES_NO_TYPE ) )
		return FALSE;

	wy_printf("Place all cards in the idle state ok!!!!\n");

	// When the card is in idle state,the host shall issue CMD8 before ACMD41 --- 这点很重要，不然不能识别卡
	SDHC_IssueCommand( sCh, 8, 0x1AA, SDHC_CMD_AC_TYPE, SDHC_RES_R7_TYPE );
	wy_printf("SDHC_RSP0: %x\n", SDInp32( sCh->m_uBaseAddr+SDHC_RSP0));
	wy_printf("SDHC_RSP1: %x\n", SDInp32( sCh->m_uBaseAddr+SDHC_RSP1));
	wy_printf("SDHC_RSP2: %x\n", SDInp32( sCh->m_uBaseAddr+SDHC_RSP2));
	wy_printf("SDHC_RSP3: %x\n", SDInp32( sCh->m_uBaseAddr+SDHC_RSP3));		

	for(i=0; i<500; i++)
	{
		// 发CMD55 ---- 发所有的ACMD前，都必须先发CMD55
		SDHC_IssueCommand( sCh, 55, 0, SDHC_CMD_AC_TYPE, SDHC_RES_R1_TYPE );

		// 发ACMD41 ---- 告诉SD卡，主机能够提供的电压范围是(Ocr:2.7V~3.6V)
		SDHC_IssueCommand( sCh, 41, 0x40ff8000, SDHC_CMD_BCR_TYPE, SDHC_RES_R3_TYPE );
		
		// 解析ACMD41的应答信号(注意:因为A8的寄存器最多只有32位，但是应答信号可以有128位，所以用了4个寄存器组装起来)
		if (SDInp32(sCh->m_uBaseAddr+SDHC_RSP0)&(unsigned int)((unsigned)0x1<<31)) // 检测OCR的bit31 ---- 看是否上电成功，如果成功，才执行该if分支
		{
			wy_printf("finished the power up routine!!!!\n");			
			...
			if(OCR&(0x1<<30)) {
				...
				wy_printf("High Capacity Card\n");
			}
			else{
				sCh->m_eTransMode = SDHC_BYTE_MODE;
				wy_printf("Normal Card\n");
			}
...
U8 SDHC_GetSdScr(SDHC* sCh) {
	...
	SDHC_SetBlockSizeReg( sCh, 7, 8); //设置CPU的BLKSIZE0寄存器，设置块大小为8字节
	SDHC_SetBlockCountReg( sCh, 1); //只读写一块
	SDHC_SetTransferModeReg(0, 1, 0, 0, 0, sCh); //设为读模式
	sCh->m_uRemainBlock = 1;
	sCh->m_uBufferPtr = buffer;

	// 发CMD55 ---- 发ACMD前必须先发CMD55
	if (!SDHC_IssueCommand( sCh, 55, sCh->m_uRca<<16, SDHC_CMD_AC_TYPE, SDHC_RES_R1_TYPE ) )
		return FALSE;

	// 发ACMD51 ---- Reads the SD Configuration Register(SCR)
...
void SDHC_WriteOneBlock(U32 uBufAddr) {
	...
	int block_size;
	int i;
	block_size = SDInp16( (sCh)->m_uBaseAddr + SDHC_BLK_SIZE ) & 0xFFF; //读取块大小
	...
	{
		SDOutp32( sCh->m_uBaseAddr+SDHC_BUF_DAT_PORT, *source_Ptr++); //发数据
	}
	sCh->m_uRemainBlock--;
...
U8 SDHC_WriteBlocks(U32 uStBlock, U16 uBlocks, U32* uBufAddr) {
	...
	sCh->m_uBufferPtr = uBufAddr; // 源

	if ( !SDHC_WaitForCard2TransferState( sCh ) ) //等待卡进入发送模式
		return 3;
	
	SDHC_SetBlockSizeReg(sCh, 7, 512); // Maximum DMA Buffer Size, Block Size is 512Bytes
	SDHC_SetBlockCountReg(sCh, uBlocks); // Block Numbers to Write
	...
	if( sCh->m_eOpMode == SDHC_SDMA_MODE ) {
		...
		// Interrupt or Polling mode.执行该分支(1,0,1,1,0) ---- 多块操作|写操作|自动发结束命令|块计数使能|DMA禁止
...
U8 SDHC_IdentifyCard(SDHC* sCh) // 0xEB000000
{
	SDHC_Set_InitClock( sCh );	// initial Clock Setting.

	SDOutp8( sCh->m_uBaseAddr+SDHC_TIMEOUT_CTRL, 
		(SDInp8(sCh->m_uBaseAddr+SDHC_TIMEOUT_CTRL)&(0xF<<4))|0xE);	// Timeout setting.设置超时时间
	SDOutp8( sCh->m_uBaseAddr+SDHC_HOST_CTRL, 0); // HOSTCTL0 = 0，SDMA selected.
	SDHC_SetHostCtrlSpeedMode(SDHC_NORMAL_SPEED, sCh); // NORMAL Speed mode.

	SDHC_SetSdhcInterruptEnable(0x3F7, 0xFF, 0x0, 0x0, sCh);	// except DMA interrupt

	// Check card OCR(Operation Condition Register) ---- 这里需要重点调试
	if (SDHC_SetSDOCR(sCh))
	{
		wy_printf(" SDHC_SD_CARD!!\n");
		sCh->m_eCardType = SDHC_SD_CARD;
	}
	else
	{
		wy_printf(" Normal Card and Exit!!\n");
		return FALSE;
	}

	// 发CMD2 ---- Asks any card to send the CID numbers on the CMD line (any card that is connected to the host will respond)
	SDHC_IssueCommand( sCh, 2, 0, SDHC_CMD_BCR_TYPE, SDHC_RES_R2_TYPE );

	#if 0
	wy_printf("Product Name : %c%c%c%c%c%c\n",((SDInp32(sCh->m_uBaseAddr+SDHC_RSP2)>>24)&0xFF),
	...
	#endif
	wy_printf(" the CID of the card: "); // 000353445355303447805255695900C2
	wy_printf("%x", SDInp32( sCh->m_uBaseAddr+SDHC_RSP3));		
	wy_printf("%x", SDInp32( sCh->m_uBaseAddr+SDHC_RSP2));
	wy_printf("%x", SDInp32( sCh->m_uBaseAddr+SDHC_RSP1));
	wy_printf("%x\n", SDInp32( sCh->m_uBaseAddr+SDHC_RSP0)); // 2012年2月生产
	
	sCh->m_uRca= (sCh->m_eCardType==SDHC_MMC_CARD) ? (1): (0);

	// 发CMD3 ---- Send RCA(即相应的卡的地址). It places the card in the STBY state
	// wy_printf("RCA=0x%x\n", sCh->m_uRca);
	...
	// 发CMD9 ---- 让已经被地址标识的卡发送它自己的说明(CSD寄存器)(如大小等)
...
U8 SDHC_SetSdCardSpeedMode(SDHC_SpeedMode eSpeedMode, SDHC* sCh)
{
	...
	SDHC_SetBlockSizeReg(sCh, 7, 64); // 在主机的角度设置块大小为64字节
	SDHC_SetBlockCountReg(sCh, 1); // 读写一块
	SDHC_SetTransferModeReg(0, 1, 0, 0, 0,sCh); // 读模式
	sCh->m_uRemainBlock = 1;
	sCh->m_uBufferPtr = buffer;
	...
	// 发CMD6 ---- 切换卡的速度模式和卡的功能(以便兼容早期的(v1.0等)主机控制器)
	if( !SDHC_IssueCommand( sCh, 6, uArg, SDHC_CMD_ADTC_TYPE, SDHC_RES_R1_TYPE ) ) { // 设置成Normal Speed
		puts("CMD6 fail\n");
		return FALSE;
	}
	SDHC_ReadOneBlock( (U32)sCh->m_uBufferPtr );
	...
	if ( buffer[3] & (1<<9) ) { // Function Group 1 <- access mode.
		wy_printf( "This Media Card support high speed mode.\n" );
	}
	else {
		wy_printf( "This Media Card can't support high speed mode.\n" );
	}
...
void SDHC_SetSdClockOnOff(U8 uOnOff, SDHC* sCh)
{
	if (uOnOff == FALSE)
	{
	...
	else // 开
 	{
		...
		while ( !( SDInp16( sCh->m_uBaseAddr+SDHC_CLK_CTRL ) & (1<<3) ) );// 直到供给SD卡的时钟稳定了，才跳出该循环
	}
}
...
{
	if(speed==SDHC_NORMAL_SPEED) { //执行该分支
		SDOutp32( sCh->m_uBaseAddr+SDHC_CONTROL2, (1<<30)|(0<<15)|(0<<14)|(0x1<<8)|(sCh->m_eClockSource<<4) ); //使能SDCLK时钟,时钟源选为HCLK
	...
	SDOutp16( sCh->m_uBaseAddr+SDHC_CLK_CTRL,
		(SDInp16(sCh->m_uBaseAddr+SDHC_CLK_CTRL)&(~(0xff<<8)))|(sCh->m_uClockDivision<<8)|(1<<0) ); // sCh->m_uClockDivision = 2 --- sdclk = 95.3/4 = 23.825
...
U8 SDHC_SetDataTransferWidth( SDHC* sCh)
{
	...
	if ( sCh->m_eCardType == SDHC_SD_CARD ) {
		...
		// 发ACMD6 - 设置位宽 0->1bit, 2->4bit(从卡的角度，设置为4位了)
...
void SDHC_Set_InitClock( SDHC* sCh ) {
	SDOutp32( sCh->m_uBaseAddr+SDHC_CONTROL2, (0x0<<15)|(0x0<<14)|(0x1<<8)|(SDHC_HCLK<<4) ); // CONTROL2_0 = 使能SDCLK | HCLK作为基础时钟(HCLKP =95.3MHz)
	SDOutp32( sCh->m_uBaseAddr+SDHC_CONTROL3, (0<<31) | (0<<23) | (0<<15) | (0<<7) ); // CONTROL3_0 有什么作用?

	// SDCLK Value Setting + Internal Clock Enable
	SDOutp16( sCh->m_uBaseAddr+SDHC_CLK_CTRL,
		(SDInp16(sCh->m_uBaseAddr+SDHC_CLK_CTRL)&(~(0xff<<8)))|(0x80<<8)|(1<<0) ); // CLKCON0 ---- SDCLK = 95.3MHz / 256 = 372.26KHz

	// CheckInternalClockStable
	while (!(SDInp16( sCh->m_uBaseAddr+SDHC_CLK_CTRL )&0x2)); // 直到内部时钟稳定才跳出该循环
	SDHC_SetSdClockOnOff( TRUE, sCh); //使能SD卡的时钟
}
...
void SDHC_ReadOneBlock(U32 uBufAddr) {
	...
	block_size = (block_size+3) >> 2;	// block_size = (block_size+3) / 4; 用于4字节对齐用
...
void SDHC_DisplayCardInformation(SDHC * sCh)
{
	...
	sCh->m_sReadBlockLen = (U16)((SDInp32( sCh->m_uBaseAddr+SDHC_RSP2 )>>8) & 0xf); // read data block length = 固定的0x9
	sCh->m_sReadBlockPartial = (U16)((SDInp32( sCh->m_uBaseAddr+SDHC_RSP2 )>>7) & 0x1); // 固定是0，表明不能读一块的局部，只能整块的读
	//wy_printf("ReadBlockPartial: 0x%x\n",sCh->m_sReadBlockPartial);
	sCh->m_sCSize = (U16)((SDInp32( sCh->m_uBaseAddr+SDHC_RSP1 ) >> 8) & 0x3fffff);
	//wy_printf("CSize: 0x%x\n",sCh->m_sCSize);
	
	//sCh->m_sCSizeMult = (U16)((SDInp32( sCh->m_uBaseAddr+SDHC_RSP1 )>>7)&0x7);
	
	CardSize = (sCh->m_sCSize+1) * 512 / 1024;
	OneBlockSize = 512;
	//SDHC_global_card_size = ((U32)(1<<sCh->m_sReadBlockLen))*(sCh->m_sCSize+1)*(1<<(sCh->m_sCSizeMult+2))/512;

	wy_printf("One Block Size: 0x%xByte\n",OneBlockSize);
	wy_printf("Total Card Size: 0x%xMByte\n",CardSize);     // 3.69 G
}

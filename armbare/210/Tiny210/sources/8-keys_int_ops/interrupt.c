...
static int keys_int_init_ll(void);
...
extern void keys_irq_handler(void);
...
/* Keys interrupt sources init */
static int keys_int_init_ll(void)
{
	GPH2CON	|= (0xF<<12 | 0xF<<8 | 0xF<<4 | 0xF);
	GPH3CON	|= (0xF<<12 | 0xF<<8 | 0xF<<4 | 0xF);

	EXT_INT_2_CON |= (0x4<<12 | 0x4<<8 | 0x4<<4 | 0x4);
	EXT_INT_3_CON |= (0x4<<12 | 0x4<<8 | 0x4<<4 | 0x4);

	EXT_INT_2_MASK &= ~(0x1<<3 | 0x1<<2 | 0x1<<1 | 0x1);
	EXT_INT_3_MASK &= ~(0x1<<3 | 0x1<<2 | 0x1<<1 | 0x1);
...
static int IC_init_ll(void)
{
	...
	vicIntSelectIRQ(EINT_16_31);

	/* Enable the interrupt request line of the VIC0 */
	vicIntEnable(EINT_16_31);

	/* Setup the ISR vector address */
	vicVectAddr(EINT_16_31, keys_irq_handler);

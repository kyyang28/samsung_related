...
REWTCMD:
	...
	{
		case DMA:
			isr[INT_DMA0_OFT] = DMA_end;
			INTMSK = ~(INT_DMA0);
			SDIDCON = SDIDCON | (1<<24); //YH 040227, Burst4 Enable

			DISRC0=(int)(Tx_buffer);	// Tx_buffer
			DISRCC0=(0<<1)+(0<<0);	// AHB, inc
			DIDST0=(u32)(SDIDAT);	// SDIDAT
			DIDSTC0=(1<<1)+(1<<0);	// APB, fix
			DCON0=(1<<31)+(0<<30)+(1<<29)+(0<<28)+(0<<27)+(2<<24)+(1<<23)+(1<<22)+(2<<20)+128*block;
			//handshake, sync PCLK, TC int, single tx, single service, SDI, H/W request, 
			//auto-reload off, word, 128blk*num
			DMASKTRIG0=(0<<2)+(1<<1)+0;    //no-stop, DMA0 channel on, no-sw trigger
...
RERDCMD:
	switch(mode)
	{
		case DMA:
			...
			DISRC0=(int)(SDIDAT);	// SDIDAT
			DISRCC0=(1<<1)+(1<<0);	// APB, fix
			DIDST0=(u32)(Rx_buffer);	// Rx_buffer
			DIDSTC0=(0<<1)+(0<<0);	// AHB, inc
			DCON0=(1<<31)+(0<<30)+(1<<29)+(0<<28)+(0<<27)+(2<<24)+(1<<23)+(1<<22)+(2<<20)+128*block;

			DMASKTRIG0=(0<<2)+(1<<1)+0;    //no-stop, DMA2 channel on, no-sw trigger 
